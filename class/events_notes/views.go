...
type CatatanKelasViews struct{}

func (c *CatatanKelasViews) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	switch r.Method {
	case http.MethodGet:
		...
		viewer, err := school_class_events_notes_viewer.SchoolClassEventsNotesViewerDB.GetByClassEventsNotesID(uint(classEventsID))
		if err != nil {
...
func init() {
	http.DefaultServeMux.Handle("/v1/class/events/notes/viewer", auth.AuthMiddlewareHandler(&CatatanKelasViews{}))
}
