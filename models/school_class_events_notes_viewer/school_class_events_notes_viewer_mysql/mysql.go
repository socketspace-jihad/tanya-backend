package school_class_events_notes_viewer_mysql
...
type SchoolClassEventsNotesViewerMySQL struct {
	*sql.DB
}

func (s *SchoolClassEventsNotesViewerMySQL) Save(data *school_class_events_notes_viewer.SchoolClassEventsNotesViewerData) error {
	tx, err := s.DB.Begin()
	if err != nil {
		tx.Rollback()
		return err
...
func (s *SchoolClassEventsNotesViewerMySQL) GetByID(uint) (*school_class_events_notes_viewer.SchoolClassEventsNotesViewerData, error) {
	return nil, nil
}

func (s *SchoolClassEventsNotesViewerMySQL) GetByClassEventsNotesID(id uint) ([]school_class_events_notes_viewer.SchoolClassEventsNotesViewerData, error) {
	rows, err := s.DB.Query(`
		SELECT
			cenv.id,
			IFNULL(tp.id,0),
			...
			IFNULL(sp.id,0),
			IFNULL(sp.name,"")
		FROM class_events_notes_viewers AS cenv
		...
		WHERE cenv.class_events_notes_id = ?
	`, id)
	if err != nil {
		return nil, err
	}
	var events []school_class_events_notes_viewer.SchoolClassEventsNotesViewerData
	for rows.Next() {
		event := school_class_events_notes_viewer.SchoolClassEventsNotesViewerData{
...
func init() {
	...
	school_class_events_notes_viewer.SchoolClassEventsNotesViewerDB = &SchoolClassEventsNotesViewerMySQL{
		DB: db,
	}
}
